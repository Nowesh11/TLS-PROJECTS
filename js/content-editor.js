...
class ContentEditor {
    ...
    async loadPageContent(page) {
        ...
        try {
            ...
            console.log("[LOAD PAGE CONTENT] Response headers:", response.headers);
            ...
            if (error.name === "AbortError") {
                ...
                if (retryCount < this.maxRetries) {
                    ...
                    console.log("[LOAD PAGE CONTENT] Retrying... Attempt " + (retryCount + 1) + "/" + this.maxRetries);
                    setTimeout(() => this.loadPageContent(page), 1000 * (retryCount + 1));
                    return;
                } else {
    ...
    async loadActivityLog(page) {
        try {
            ...
            const response = await fetch(this.apiBaseUrl + "/api/admin/activity-log?page=" + page, {
                headers: {
                    "Authorization": "Bearer " + token
                }
            });
            ...
            if (response.ok) {
                ...
                console.log("[ACTIVITY LOG] Loaded " + activities.length + " activities for " + page);
            }
        } catch (error) {
