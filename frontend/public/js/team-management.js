...
async function saveTeamMember(event) {
    ...
    const nameTa = formData.get('name_ta');
    const bioEn = formData.get('bio_en');
    const bioTa = formData.get('bio_ta');
    
    if (nameEn || nameTa) {
        const nameData = {};
        if (nameEn) nameData.en = nameEn;
        if (nameTa) nameData.ta = nameTa;
        formData.set('name', JSON.stringify(nameData));
        formData.delete('name_en');
        formData.delete('name_ta');
    }
    
    if (bioEn || bioTa) {
        const bioData = {};
        if (bioEn) bioData.en = bioEn;
        if (bioTa) bioData.ta = bioTa;
        formData.set('bio', JSON.stringify(bioData));
        formData.delete('bio_en');
        formData.delete('bio_ta');
    }
...
function populateTeamMemberForm(member) {
    ...
    form.name_en.value = member.name?.en || member.name_en || '';
    form.name_ta.value = member.name?.ta || member.name_ta || '';
    form.role.value = member.role || '';
    form.email.value = member.email || '';
    form.phone.value = member.phone || '';
    form.order_num.value = member.order_num || 1;
    form.bio_en.value = member.bio?.en || member.bio_en || '';
    form.bio_ta.value = member.bio?.ta || member.bio_ta || '';
