...
if (!window.domCache) {
    ...
    script1.src = './js/dom-cache.js';
    document.head.appendChild(script1);
}
if (!window.eventManager) {
    const script2 = document.createElement('script');
    script2.src = './js/event-manager.js';
    document.head.appendChild(script2);
}
if (!window.eventCleanupManager) {
    const script3 = document.createElement('script');
    script3.src = './js/event-cleanup-manager.js';
    document.head.appendChild(script3);
}
if (!window.sharedUtilities) {
    const script4 = document.createElement('script');
    script4.src = './js/shared-utilities.js';
...
// Safe element access wrapper with DOM caching
function safeGetElement(id) {
    try {
        // Use DOM cache if available, fallback to direct query
        const element = window.domCache ? window.domCache.get(`#${id}`) : document.getElementById(id);
        if (!element) {
...
// Safe addEventListener wrapper with event management and cleanup tracking
function safeAddEventListener(elementId, event, handler) {
    ...
    if (element) {
        try {
            ...
            // Use event manager if available for better cleanup
            if (window.eventManager) {
                listenerId = window.eventManager.on(element, event, handler, {}, 'admin');
            } else {
                element.addEventListener(event, handler);
                listenerId = true;
            }
            
            // Register with cleanup manager for memory leak prevention
            if (window.eventCleanupManager && listenerId) {
                window.eventCleanupManager.registerListener(element, event, handler);
            }
...
// Table sorting functionality with performance optimizations
class TableSorter {
    constructor() {
        this.sortState = {}; // Track sort direction for each table
        this.cachedTables = new Map(); // Cache table references
        this.eventListeners = new Map(); // Track event listeners for cleanup
    }

    initializeSorting() {
        // Add sorting to all data tables using cached queries
        const tables = window.domCache ? 
            window.domCache.get('.data-table table', false) : 
            document.querySelectorAll('.data-table table');
    ...
    addSortingToTable(table) {
        ...
        headers.forEach((header, index) => {
            ...
            // Use event manager for better cleanup if available
            const clickHandler = () => this.sortTable(table, index, tableId);
            
            if (window.eventManager) {
                const listenerId = window.eventManager.on(header, 'click', clickHandler, {}, 'table-sorter');
                this.eventListeners.set(`${tableId}_${index}`, listenerId);
            } else {
    ...
    sortTable(table, columnIndex, tableId) {
        // Use cached table elements for better performance
        const cachedTable = this.cachedTables.get(tableId);
        const tbody = cachedTable?.tbody || table.querySelector('tbody');
        const headers = cachedTable?.headers || Array.from(table.querySelectorAll('thead th'));
        ...
        const currentSort = this.sortState[sortKey] || 'none';
        const newSort = (currentSort === 'none' || currentSort === 'desc') ? 'asc' : 'desc';
        
        // Clear all sort indicators in this table (use cached headers)
        headers.forEach(h => {
        ...
        // Optimized sorting with better type detection
        ...
        rows.sort((a, b) => {
            ...
            const aNum = this.parseNumber(aText);
            const bNum = this.parseNumber(bText);
            ...
            if (aNum !== null && bNum !== null) {
                ...
                const aDate = this.parseDate(aText);
                const bDate = this.parseDate(bText);
    ...
    // Helper method for better number parsing
    parseNumber(text) {
    ...
    // Helper method for better date parsing
    parseDate(text) {
    ...
    try {
        ...
        if (tinymce.init) {
            await tinymce.init({
                ...
                // Paste handling (images disabled)
...
/**
 * Make API calls with authentication - using shared utilities
 */
if (typeof window.apiCall !== 'function') {
    // Use shared utilities API call method with enhanced error handling
    window.apiCall = async function(endpoint, options = {}) {
        if (window.sharedUtilities) {
            try {
                return await window.sharedUtilities.apiCall(endpoint, options);
            } catch (error) {
                ...
                if (error.message.includes('401')) {
                    ...
                    if (window.tokenManager) {
                        try {
                            ...
                            console.log('TokenManager refresh failed:', refreshError);
                        }
                    }
                    
                    if (!refreshed && typeof retryAuthentication === 'function') {
        ...
        // Fallback for when shared utilities isn't loaded yet
        console.warn('Shared utilities not available, using basic fallback API call');
...
/**
 * Show section creator modal - using shared utilities
 */
function showSectionCreator(selectedPage = null) {
    ...
    // Use shared utilities for modal management
    if (window.sharedUtilities) {
        const success = window.sharedUtilities.manageModal('sectionCreatorModal', true, {
            resetForm: true,
            focusFirst: true
        });
...
/**
 * Close section creator modal - using shared utilities
 */
function closeSectionCreator() {
    ...
    // Use shared utilities for modal management
    if (window.sharedUtilities) {
        window.sharedUtilities.manageModal('sectionCreatorModal', false);
    }
}
...
function handleSectionCreation(event) {
    ...
    const sectionData = {
        ...
        sectionKey: `${page}.${sectionName.trim().toLowerCase().replace(/\s+/g, '-')}`,
        ...
        title: {
            en: formData.get('titleEn') || sectionName.trim(),
            ta: formData.get('titleTa') || ''
        },
        content: {
            en: formData.get('contentEn') || '',
            ta: formData.get('contentTa') || ''
        },
        subtitle: {
            en: formData.get('subtitleEn') || '',
            ta: formData.get('subtitleTa') || ''
        },
        buttonText: {
            en: formData.get('buttonTextEn') || '',
            ta: formData.get('buttonTextTa') || ''
        },
...
async function createSection(sectionData) {
    try {
        ...
        const apiEndpoint = `/api/content/pages/${sectionData.page}/sections`;
...
/**
 * Switch tabs in section creator modal
 */
function switchTab(tabName) {
    ...
    if (clickedBtn) {
        ...
        clickedBtn.style.background = 'var(--bg-accent)';
        clickedBtn.style.color = 'var(--text-primary)';
        clickedBtn.style.boxShadow = 'var(--shadow-lg)';
    }
}
 ...
 * Initialize section type card selection
 */
function initSectionTypeSelection() {
    // Add click event listeners to section type cards
...
function toggleEditorLanguage(editor) {
    ...
    if (isTamilMode) {
        ...
        showNotification('Switched to Tamil input mode', 'info');
    }
}
    ...
    try {
        switch (sectionId) {
            ...
            case 'media-management':
                ...
                break;
            case 'posters':
                console.log('Loading posters data...');
                if (typeof loadPostersData === 'function') {
                    await loadPostersData();
                } else {
                    console.warn('loadPostersData function not found - ensure posters-management.js is loaded');
                }
                break;
            case 'slideshow':
                console.log('Loading slideshow data...');
                if (typeof loadSlideshowData === 'function') {
                    await loadSlideshowData();
                } else {
                    console.warn('loadSlideshowData function not found');
                }
                break;
            case 'file-storage':
                console.log('Loading file storage data...');
                if (typeof loadFileStorageData === 'function') {
                    await loadFileStorageData();
                } else {
                    console.warn('loadFileStorageData function not found');
                }
                break;
            case 'announcements':
            ...
            case 'chats':
                ...
                if (typeof loadChatsData === 'function') {
                    await loadChatsData();
                } else {
                    console.warn('loadChatsData function not found');
                }
                break;
            default:
...
function showProjectModal(project = null) {
    ...
    if (project) {
        ...
        form.querySelector('[name="title.en"]').value = project.title?.en || project.title || '';
        form.querySelector('[name="title.ta"]').value = project.title?.ta || project.titleTamil || '';
        form.querySelector('[name="description.en"]').value = project.description?.en || project.description || '';
        form.querySelector('[name="description.ta"]').value = project.description?.ta || project.descriptionTamil || '';
        ...
        form.querySelector('[name="goals.en"]').value = project.goals?.en || project.goals || '';
        form.querySelector('[name="goals.ta"]').value = project.goals?.ta || project.goalsTamil || '';
        form.querySelector('[name="achievements.en"]').value = project.achievements?.en || project.achievements || '';
        form.querySelector('[name="achievements.ta"]').value = project.achievements?.ta || project.achievementsTamil || '';
...
async function loadActivities() {
    try {
        ...
        const response = await apiCall(`/api/activities?lang=${language}`);
...
function showActivityModal(activity = null) {
    ...
    if (activity) {
        ...
        // Populate form fields
        form.querySelector('[name="title.en"]').value = activity.title?.en || activity.title || '';
        form.querySelector('[name="title.ta"]').value = activity.title?.ta || activity.titleTamil || '';
        form.querySelector('[name="description.en"]').value = activity.description?.en || activity.description || '';
        form.querySelector('[name="description.ta"]').value = activity.description?.ta || activity.descriptionTamil || '';
        ...
        form.querySelector('[name="goals.en"]').value = activity.goals?.en || activity.goals || '';
        form.querySelector('[name="goals.ta"]').value = activity.goals?.ta || activity.goalsTamil || '';
        form.querySelector('[name="achievements.en"]').value = activity.achievements?.en || activity.achievements || '';
        form.querySelector('[name="achievements.ta"]').value = activity.achievements?.ta || activity.achievementsTamil || '';
...
async function loadInitiatives() {
    try {
        const language = localStorage.getItem('selectedLanguage') || 'en';
        const response = await apiCall(`/api/initiatives?lang=${language}`);
...
function showInitiativeModal(initiative = null) {
    ...
    if (initiative) {
        ...
        // Populate form fields
        form.querySelector('[name="title.en"]').value = initiative.title?.en || initiative.title || '';
        form.querySelector('[name="title.ta"]').value = initiative.title?.ta || initiative.titleTamil || '';
        form.querySelector('[name="description.en"]').value = initiative.description?.en || initiative.description || '';
        form.querySelector('[name="description.ta"]').value = initiative.description?.ta || initiative.descriptionTamil || '';
        ...
        form.querySelector('[name="goals.en"]').value = initiative.goals?.en || initiative.goals || '';
        form.querySelector('[name="goals.ta"]').value = initiative.goals?.ta || initiative.goalsTamil || '';
        form.querySelector('[name="achievements.en"]').value = initiative.achievements?.en || initiative.achievements || '';
        form.querySelector('[name="achievements.ta"]').value = initiative.achievements?.ta || initiative.achievementsTamil || '';
...
function initFileUpload() {
    // File upload functionality removed except for logo uploads
    // Logo uploads are handled separately via uploadGlobalLogo() function
    
    // Add event listeners for form submissions
    initFormHandlers();
}

// Drag and drop functionality removed - only logo uploads supported

// File selection handling removed - only logo uploads supported

// File preview functions removed - only logo uploads supported
...
/**
 * Handle content form submission - using shared utilities
 * @param {Event} event - The form submit event
 */
/**
 * Validate bilingual content before submission
 * @param {FormData} formData - The form data to validate
 * @returns {Object} - Validation result with isValid and errors
 */
function validateBilingualContent(formData) {
    const errors = [];
    const requiredFields = ['title', 'content', 'text'];
    const optionalFields = ['subtitle', 'buttonText'];
    
    // Check if we have both English and Tamil for required fields
    for (const field of requiredFields) {
        const enValue = formData.get(`content[en][${field}]`);
        const taValue = formData.get(`content[ta][${field}]`);
        
        if (enValue && enValue.trim()) {
            if (!taValue || !taValue.trim()) {
                errors.push(`Tamil translation is required for ${field}`);
            }
        }
        
        if (taValue && taValue.trim()) {
            if (!enValue || !enValue.trim()) {
                errors.push(`English content is required for ${field}`);
            }
        }
    }
    
    // Check for at least one complete bilingual pair
    let hasCompleteContent = false;
    for (const field of [...requiredFields, ...optionalFields]) {
        const enValue = formData.get(`content[en][${field}]`);
        const taValue = formData.get(`content[ta][${field}]`);
        
        if (enValue && enValue.trim() && taValue && taValue.trim()) {
    ...
    if (!hasCompleteContent) {
        errors.push('At least one field must have both English and Tamil content');
    }
...
/**
 * Enhanced content form submission with bilingual validation
 * @param {Event} event - The form submit event
 */
async function handleContentFormSubmit(event) {
    ...
    const validation = validateBilingualContent(formData);
    
    if (!validation.isValid) {
        // Show validation errors
        const errorMessage = 'Bilingual validation failed:\n' + validation.errors.join('\n');
        showNotification(errorMessage, 'error');
        
        // Highlight missing fields
        highlightMissingBilingualFields(form, validation.errors);
        return;
    }
    ...
    try {
        if (window.sharedUtilities) {
            await window.sharedUtilities.handleFormSubmission(event, {
                endpoint: '/api/website-content',
                method: 'POST',
                successMessage: 'Website content saved successfully with bilingual validation',
                errorMessage: 'Failed to save content'
            });
        } else {
            // Fallback implementation with enhanced error handling
            const response = await apiCall('/api/website-content', {
                method: 'POST',
                body: formData
            });
            
            showNotification('Website content saved successfully with bilingual validation', 'success');
        ...
        if (error.message && error.message.includes('bilingual')) {
...
/**
 * Highlight missing bilingual fields in the form
 * @param {HTMLFormElement} form - The form element
 * @param {Array} errors - Array of validation errors
 */
function highlightMissingBilingualFields(form, errors) {
    // Remove existing highlights
    const highlightedFields = form.querySelectorAll('.bilingual-error');
    highlightedFields.forEach(field => {
        field.classList.remove('bilingual-error');
        field.style.borderColor = '';
    });
    
    // Add highlights for missing fields
    errors.forEach(error => {
        const fieldName = error.match(/for (\w+)/);
        if (fieldName && fieldName[1]) {
            const field = fieldName[1];
            
            // Highlight both English and Tamil fields
            const enField = form.querySelector(`[name="content[en][${field}]"]`);
            const taField = form.querySelector(`[name="content[ta][${field}]"]`);
            
            if (enField) {
                enField.classList.add('bilingual-error');
                enField.style.borderColor = '#dc3545';
            }
            
            if (taField) {
                taField.classList.add('bilingual-error');
                taField.style.borderColor = '#dc3545';
            }
        }
    });
    
    // Scroll to first error field
    const firstErrorField = form.querySelector('.bilingual-error');
    if (firstErrorField) {
    ...
    return `
        ...
        <form id="bookForm" class="modern-form" enctype="multipart/form-data">
            <div class="modern-form-group">
                ...
                <input type="text" class="modern-form-input" id="title" name="title.en" placeholder="Enter book title" value="${book?.title?.en || book?.title || ''}" required>
            </div>
            
            <div class="modern-form-group">
                <label class="modern-form-label">Tamil Title</label>
                <input type="text" class="modern-form-input" id="titleTamil" name="title.ta" placeholder="Enter Tamil title" value="${book?.title?.ta || book?.titleTamil || ''}" style="font-family: 'Noto Sans Tamil', sans-serif;">
            </div>
            
            <div class="modern-form-group">
                <label class="modern-form-label required">Author (English)</label>
                <input type="text" class="modern-form-input" id="author" name="author.en" placeholder="Enter author name" value="${book?.author?.en || book?.author || ''}" required>
            </div>
            
            <div class="modern-form-group">
                <label class="modern-form-label">Tamil Author</label>
                <input type="text" class="modern-form-input" id="authorTamil" name="author.ta" placeholder="Enter Tamil author name" value="${book?.author?.ta || book?.authorTamil || ''}" style="font-family: 'Noto Sans Tamil', sans-serif;">
            </div>
            ...
            <div class="modern-form-group">
                ...
                <textarea class="modern-form-textarea" id="description" name="description.en" placeholder="Enter book description" required>${book?.description?.en || book?.description || ''}</textarea>
            </div>
            
            <div class="modern-form-group">
                <label class="modern-form-label">Description (Tamil)</label>
                <textarea class="modern-form-textarea" id="descriptionTamil" name="description.ta" placeholder="Enter Tamil description" style="font-family: 'Noto Sans Tamil', sans-serif;">${book?.description?.ta || ''}</textarea>
            </div>
...
async function getEbookModalContent(id = null) {
    ...
    return `
        ...
        <form id="ebookForm" class="modern-form" enctype="multipart/form-data">
            <div class="modern-form-group">
                <label class="modern-form-label required">Title (English)</label>
                <input type="text" class="modern-form-input" id="title" name="title.en" placeholder="Enter e-book title" value="${ebook?.title?.en || ebook?.title || ''}" required>
            </div>
            
            <div class="modern-form-group">
                <label class="modern-form-label">Tamil Title</label>
                <input type="text" class="modern-form-input" id="titleTamil" name="title.ta" placeholder="Enter Tamil title" value="${ebook?.title?.ta || ebook?.titleTamil || ''}" style="font-family: 'Noto Sans Tamil', sans-serif;">
            </div>
            
            <div class="modern-form-group">
                <label class="modern-form-label required">Author (English)</label>
                <input type="text" class="modern-form-input" id="author" name="author.en" placeholder="Enter author name" value="${ebook?.author?.en || ebook?.author || ''}" required>
            </div>
            
            <div class="modern-form-group">
                <label class="modern-form-label">Tamil Author</label>
                <input type="text" class="modern-form-input" id="authorTamil" name="author.ta" placeholder="Enter Tamil author name" value="${ebook?.author?.ta || ebook?.authorTamil || ''}" style="font-family: 'Noto Sans Tamil', sans-serif;">
            </div>
            ...
            <div class="modern-form-group">
                ...
                <textarea class="modern-form-textarea" id="description" name="description.en" placeholder="Enter e-book description">${ebook?.description?.en || ebook?.description || ''}</textarea>
            </div>
            
            <div class="modern-form-group">
                <label class="modern-form-label">Tamil Description</label>
                <textarea class="modern-form-textarea" id="descriptionTamil" name="description.ta" placeholder="Enter Tamil description" style="font-family: 'Noto Sans Tamil', sans-serif;">${ebook?.description?.ta || ebook?.descriptionTamil || ''}</textarea>
            </div>
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: [
                                ...
                                'var(--warning-color)', // Yellow for ebooks
                                'var(--success-color)', // Green for projects
                                'var(--error-color)'  // Red for messages
                            ],
                            borderWidth: 2,
...
function initContentManagement() {
    if (window.sharedUtilities) {
        window.sharedUtilities.initializeModules([
            {
                name: 'ContentManagement',
                dependencies: ['contentEditor'],
                initFunction: initContentManagementCore,
                timeout: 2000
            }
        ]);
    } else {
...
async function loadWebsiteContent() {
    try {
        ...
        const transformedContent = {
            ...
            logo: allContent.global?.logo?.image_url || 'assets/logo.png'
        };
    ...
    const formHTML = `
        <form id="bookForm" ${book ? `data-book-id="${book._id}"` : ''}>
            <div class="form-group">
                ...
                <input type="text" name="title.en" class="form-input" value="${book ? (book.title?.en || book.title || '') : ''}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Title (Tamil)</label>
                <input type="text" name="title.ta" class="form-input" value="${book ? (book.title?.ta || book.titleTamil || '') : ''}" style="font-family: 'Noto Sans Tamil', sans-serif;">
            </div>
            <div class="form-group">
                <label class="form-label">Author (English)</label>
                <input type="text" name="author.en" class="form-input" value="${book ? (book.author?.en || book.author || '') : ''}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Author (Tamil)</label>
                <input type="text" name="author.ta" class="form-input" value="${book ? (book.author?.ta || book.authorTamil || '') : ''}" style="font-family: 'Noto Sans Tamil', sans-serif;">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea name="description.en" class="form-textarea" rows="4" required>${book ? (book.description?.en || book.description || '') : ''}</textarea>
            </div>
            <div class="form-group">
                <label class="form-label">Description (Tamil)</label>
                <textarea name="description.ta" class="form-textarea" rows="4" style="font-family: 'Noto Sans Tamil', sans-serif;">${book ? (book.description?.ta || '') : ''}</textarea>
            </div>
            <div class="form-group">
...
function showEbookModal(ebook = null) {
    ...
    const formHTML = `
        <form id="ebookForm" ${ebook ? `data-ebook-id="${ebook._id}"` : ''}>
            <div class="form-group">
                <label class="form-label">Title (English)</label>
                <input type="text" name="title.en" class="form-input" value="${ebook ? (ebook.title?.en || ebook.title || '') : ''}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Title (Tamil)</label>
                <input type="text" name="title.ta" class="form-input" value="${ebook ? (ebook.title?.ta || ebook.titleTamil || '') : ''}" style="font-family: 'Noto Sans Tamil', sans-serif;">
            </div>
            <div class="form-group">
                <label class="form-label">Author (English)</label>
                <input type="text" name="author.en" class="form-input" value="${ebook ? (ebook.author?.en || ebook.author || '') : ''}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Author (Tamil)</label>
                <input type="text" name="author.ta" class="form-input" value="${ebook ? (ebook.author?.ta || ebook.authorTamil || '') : ''}" style="font-family: 'Noto Sans Tamil', sans-serif;">
            </div>
            <div class="form-group">
                <label class="form-label">Category</label>
                <select name="category" class="form-input" required>
            ...
            <div class="form-group">
                <label class="form-label">Description (English)</label>
                <textarea name="description.en" class="form-textarea" rows="4" required>${ebook ? (ebook.description?.en || ebook.description || '') : ''}</textarea>
            </div>
            <div class="form-group">
                <label class="form-label">Description (Tamil)</label>
                <textarea name="description.ta" class="form-textarea" rows="4" style="font-family: 'Noto Sans Tamil', sans-serif;">${ebook ? (ebook.description?.ta || '') : ''}</textarea>
            </div>
            <div class="form-group">
                <label class="form-label">Cover Image</label>
                <input type="file" name="coverImage" class="form-input" accept="image/*">
...
async function saveBook(form) {
    ...
    // Handle bilingual fields - convert nested field names to proper structure
    const bilingualFields = ['title', 'author', 'description'];
    bilingualFields.forEach(field => {
        const enValue = formData.get(`${field}.en`);
        const taValue = formData.get(`${field}.ta`);
        
        if (enValue || taValue) {
            // Remove the nested field names
            formData.delete(`${field}.en`);
            formData.delete(`${field}.ta`);
            
            // Add as JSON string for proper backend processing
            const bilingualData = {};
            if (enValue) bilingualData.en = enValue;
            if (taValue) bilingualData.ta = taValue;
            formData.set(field, JSON.stringify(bilingualData));
        }
    });
                        ...
                        <div class="content-section-card">
                            ...
                            <form class="content-form blue-gradient-form" onsubmit="saveContentSection(event, 'logo', 'main')">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-input" name="logoImage" data-content-key="image" placeholder="/assets/logo.png">
                                </div>
                                <div class="form-group">
                ...
                <div id="footer-tab" class="content-tab">
                    ...
                    <form id="footer-form" class="blue-gradient-form" onsubmit="saveContentSection(event, 'global', 'footer')">
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-group">
                                <label class="form-label">Logo Image URL</label>
                                <input type="text" class="form-input" name="logo" placeholder="assets/logo.png">
                            </div>
                            <div class="form-group">
                                <label class="form-label">Logo Text (English)</label>
...
async function saveProject(event) {
    ...
    const bilingualFields = ['title', 'description', 'goals', 'achievements'];
    bilingualFields.forEach(field => {
        const enValue = formData.get(`${field}.en`);
        const taValue = formData.get(`${field}.ta`);
        ...
        // Remove the nested field entries
        formData.delete(`${field}.en`);
        formData.delete(`${field}.ta`);
    });
    ...
    if (titleData) {
        try {
            ...
            titleValid = titleObj.en && titleObj.en.trim();
        } catch (e) {
            titleValid = titleData && titleData.trim();
        }
    }
    
    if (descriptionData) {
        try {
            const descObj = JSON.parse(descriptionData);
            descriptionValid = descObj.en && descObj.en.trim();
        } catch (e) {
            descriptionValid = descriptionData && descriptionData.trim();
        }
    }
    
    if (!titleValid || !bureau || !descriptionValid) {
         showNotification('Please fill in all required fields (English title and description are required)', 'error');
         return;
     }
...
async function saveActivity(event) {
    ...
    const acceptingVolunteers = formData.get('acceptingVolunteers') === 'on';
    formData.set('featured', featured);
    formData.set('acceptingVolunteers', acceptingVolunteers);
    ...
    // Validate required fields
    const titleEn = JSON.parse(formData.get('title') || '{}').en;
    const descriptionEn = JSON.parse(formData.get('description') || '{}').en;
    const bureau = formData.get('bureau');
    
    if (!titleEn || !descriptionEn || !bureau) {
        showNotification('English title, description, and bureau are required', 'error');
        submitButton.innerHTML = originalButtonText;
        submitButton.disabled = false;
        return;
    }
            ...
            <div class="media-card" style="background: var(--bg-secondary); border-radius: 0.75rem; overflow: hidden; box-shadow: var(--shadow-md); transition: all 0.3s ease;">
                <div class="media-preview" style="height: 200px; background: var(--bg-tertiary); display: flex; align-items: center; justify-content: center; position: relative;">
                    ...
                    <div class="media-overlay" style="position: absolute; top: 0.5rem; right: 0.5rem; display: flex; gap: 0.25rem;">
                        <button class="btn btn-sm" onclick="downloadMedia('${file._id}')" style="background: var(--overlay-dark, rgba(0,0,0,0.7)); color: white; border: none; padding: 0.25rem 0.5rem;">
                            <i class="fas fa-download"></i>
                        </button>
                        <button class="btn btn-sm" onclick="deleteMedia('${file._id}')" style="background: var(--error-color, rgba(220,38,38,0.8)); color: white; border: none; padding: 0.25rem 0.5rem;">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </div>
