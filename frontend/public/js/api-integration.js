...
async function loadBooksFromAPI() {
    try {
        ...
        const response = await apiCall(`/api/books?lang=${language}`);
...
async function loadEbooksFromAPI() {
    try {
        const language = localStorage.getItem('selectedLanguage') || 'en';
        const response = await apiCall(`/api/ebooks?lang=${language}`);
...
async function loadProjectsFromAPI() {
    try {
        const language = localStorage.getItem('selectedLanguage') || 'en';
        const response = await apiCall(`/api/projects?lang=${language}`);
...
async function loadWebsiteContentFromAPI(page = null) {
    try {
        const language = localStorage.getItem('selectedLanguage') || localStorage.getItem('preferred_language') || 'en';
        // Use the sections endpoint for section-based content
        const endpoint = page ? `/api/website-content/sections/${page}?language=${language}` : `/api/website-content/sections/home?language=${language}`;
        ...
        console.log(`Loaded website content${page ? ` for page: ${page}` : ""} (${language}), ${Array.isArray(content) ? content.length : Object.keys(content).length} items`);
...
function transformProjectData(dbProject) {
    const categoryColors = {
        "media-public-relations": "var(--theme-primary)",
    ...
    const statusMap = {
        ...
        "completed": { label: "Completed", color: "var(--theme-primary)" },
    ...
    return [
        {
            ...
            // tamilTitle: "திருக்குறள் உரையுடன்", // Will be loaded from database
        ...
        },
        {
            ...
            // tamilTitle: "எளிய தமிழ் இலக்கணம்", // Will be loaded from database
    ...
    return [
        {
            ...
            // tamilTitle: "டிஜிட்டல் தமிழ் இலக்கியம்", // Will be loaded from database
    ...
    return [
        {
            ...
            // titleTamil: "தமிழ் மொழி பாதுகாப்பு", // Will be loaded from database
        ...
        },
        {
            ...
            // titleTamil: "டிஜிட்டல் தமிழ் காப்பகம்", // Will be loaded from database
        ...
        },
        {
            ...
            // titleTamil: "சமூக மன்றங்கள்", // Will be loaded from database
        ...
        },
        {
            ...
            // titleTamil: "தமிழ் கவிதை தொகுப்பு", // Will be loaded from database
        ...
        },
        {
            ...
            // titleTamil: "மெய்நிகர் தமிழ் வகுப்பறை", // Will be loaded from database
        ...
        },
        {
            ...
            // titleTamil: "தமிழ் இசை பாரம்பரியம்", // Will be loaded from database
