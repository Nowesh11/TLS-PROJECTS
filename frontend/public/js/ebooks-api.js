...
class EbooksAPI {
    ...
    transformEbookData(ebooks) {
        const colors = ['var(--theme-primary)', 'var(--theme-warning)', 'var(--theme-success)', 'var(--theme-accent)', 'var(--theme-error)', 'var(--theme-secondary)'];
    ...
    async downloadEbook(ebookId) {
        try {
            ...
            if (window.TamilSociety && window.TamilSociety.showNotification) {
                window.TamilSociety.showNotification(`"${ebook.title}" download started`, 'success');
            }
